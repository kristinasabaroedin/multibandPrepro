function []  = multiband_prepro_gsr(subject,runCrop,runSkullStrip,runFEATandMelodic,runFIX,runANTs,runSmoothing,runExtractTS,runGSR,runExtractTSGSR)
...
%     addpath(cfg.afnidir)
 ...
 -----------------------------------------------------------------------------------------------------------------------------
    ...
    % -----------------------------------------------------------------------------------------------------------------------------
    % Run Global Signal Regression (GSR)
    % -----------------------------------------------------------------------------------------------------------------------------
    if runGSR == 1

        display('Performing GSR')
        system([cfg.fsldir,'fslmeants -i ',cfg.fixEpi,' --label=mask.nii.gz -o ',subject,'_gsr.txt']);   
        display('Run FSL regfilt')
        cfg.gsrEpi = 'filtered_func_data_clean_gsr';
        system([cfg.fsldir, 'fsl_regfilt -i ',cfg.fixEpi,' -o ',cfg.gsrEpi,' -d ',subject,'_gsr.txt -f "1"']);
    end

    cfg.gsrEpi = 'filtered_func_data_clean_gsr_mni_smooth.nii';
    cfg.gsrDir = [cfg.derivativesdir,subject,'/gsr/'];

    % -----------------------------------------------------------------------------------------------------------------------------
    % Extract GSR time-series
    % -----------------------------------------------------------------------------------------------------------------------------

    if runExtractTSGSR == 1

        cd(cfg.gsrDir)

        % Parcellation file for time series extraction
        cfg.parcFilesGSR = {'/projects/kg98/kristina/ROIs/Gordon/Gordon_MNI_222.nii',...
                        '/projects/kg98/kristina/ROIs/Power/Power.nii',...
                        '/projects/kg98/kristina/ROIs/TriStri/TriStri.nii',...
                        '/projects/kg98/kristina/ROIs/ROIspheres/DiMartino.nii'};

        cfg.parcWeightGMGSR = {'yes',...
                            'yes',...
                            'no',...
                            'no'};

        % Set input image for time series extraction
        cfg.ExtractInGSR = [cfg.gsrDir,cfg.gsrEpi];
        ...
        cfg.roiTSGSR = [];
            ...
            % Loop over parcellation files
            for i = 1:length(cfg.parcFilesGSR)
                % Set parcellation file
                cfg.parcFileGSR = cfg.parcFilesGSR{i};
                cfg.parcNameGSR = cfg.parcFileGSR((find(cfg.parcFileGSR =='/',1,'last')+1:end-4));
                % set GM weight
                cfg.weightGMGSR = cfg.parcWeightGMGSR{i};
                % extract time series 
                cfg.roiTSGSR{i} = prepro_extractTS_FSL_GSR(cfg);
            end

            cfg = rmfield(cfg,{'parcFileGSR','weightGMGSR','parcNameGSR'});
    end   
